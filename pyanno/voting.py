...
def labels_frequency(annotations, nclasses):
    ...
    """
    annotations = np.asarray(annotations)
    allelements = np.ravel(annotations).tolist()
    allvalid = len(allelements) - allelements.count(-1)

    #label_frq = np.zeros(nclasses, dtype=float)
    #for i in np.arange(0, nclasses):
    #    i_count = allelements.count(i)
    #    i_frq = i_count / float(allvalid)
    #    label_frq[i] = i_frq

    label_frq = [allelements.count(i)/float(allvalid) \
                 for i in np.arange(0, nclasses)]

    return np.asarray(label_frq)
